...
class OtherAuctionView: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    @IBOutlet var btnSegmentArabic: UISegmentedControl!
    ...
    override func viewDidLoad() {
        ...
        if self.app.isEnglish == false {
            ...
            let arrList = ["جارية الأن", "فازت بها", "خسرتها"]
            self.btnSegmentArabic = UISegmentedControl(items: arrList)
            self.btnSegmentArabic.selectedSegmentIndex = 0
            self.btnSegmentArabic.frame = self.btnSegment.frame
            self.btnSegmentArabic.tintColor = self.btnSegment.tintColor
            self.btnSegmentArabic.addTarget(self, action: #selector(self.btnSegmentAction(_:)), for: .valueChanged)
            self.view.addSubview(self.btnSegmentArabic)
        }
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        self.seletctedIndex = self.btnSegment.selectedSegmentIndex
        if self.app.isEnglish == false {
            self.seletctedIndex = self.btnSegmentArabic.selectedSegmentIndex
        }
        if self.seletctedIndex == 0 {
            return 20
        } else {
            return 2
        }
