...
//        loadingNotification.dimBackground = true
        ...
        AppWebservice.shared.request("\(self.app.BaseURL)replace_voucher", method: .post, parameters: parameters, headers: headers, loader: true) { (statusCode, response, error) in
            if statusCode == 200 {
                self.json = response!
                print(self.json)
