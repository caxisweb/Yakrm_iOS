...
@available(iOS 11.0, *)
class HomeView: UIViewController,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,UNUserNotificationCenterDelegate
{
    //MARK:- Outlet
...
//    var loadingNotification : MBProgressHUD!
    ...
    struct Homes
    {
        ...
        init(_ voucher_id : String, _ vendor_id : String)
        {
...
//                       "Auction".localized,
        ...
        self.arrBrands = ["Cuisine".localized,
...
//        {
//            self.lblGIFTOLDER.text = "ترتيب الهدايا"
//            self.btnPopular.setTitle("  الأكثر شعبية", for: .normal)
//            self.btnDiscounted.setTitle("  الأكثر خصما", for: .normal)
//        }
        self.lblGIFTOLDER.text = "Gift Order".localized
        self.btnPopular.setTitle("Most Popular ".localized, for: .normal)
        self.btnDiscounted.setTitle("High Discounted".localized, for: .normal)
...
//        }
        ...
        self.clView!.alwaysBounceVertical = true
        ...
        self.clView!.addSubview(refresher)
    ...
    @objc func reloadListData()
    {
        if self.app.isConnectedToInternet()
        {
            self.getHomeAPI(isLoading: false, isHideLoading: false)
        }
        else
        {
            Toast(text: self.app.InternetConnectionMessage.localized).show()
    ...
    override func viewWillAppear(_ animated: Bool)
    {
        if self.arrHome.count == 0
        {
            if self.app.isConnectedToInternet()
            {
                self.getHomeAPI(isLoading: true, isHideLoading: false)
            }
            else
            {
                Toast(text: self.app.InternetConnectionMessage.localized).show()
        ...
        else
        {
            if self.app.isConnectedToInternet()
            {
                self.getHomeAPI(isLoading: true, isHideLoading: true)
            }
            else
            {
                Toast(text: self.app.InternetConnectionMessage.localized).show()
    ...
    func setCartFrame()
    {
        if self.app.strCart == "0"
        {
            self.app.strCart = ""
        }
//        self.app.strCart = "12"
        self.lblCart.text = self.app.strCart
        self.lblCart.isHidden = self.app.strCart.isEmpty
        let cartsize = self.lblCart.text!.size(withAttributes:[.font: self.lblCart.font])
        var spacewidth : CGFloat = 10.0
        if self.app.strCart.count == 3
        {
            spacewidth = 6.0
        }
        else if self.app.strCart.count == 2
        {
            spacewidth = 7.0
        }
        self.lblCart.frame = CGRect(x:self.lblCart.frame.origin.x, y:self.lblCart.frame.origin.y, width:cartsize.width + spacewidth, height:self.lblCart.frame.size.height)
        self.lblCart.layer.cornerRadius = self.lblCart.frame.size.height / 2
        self.lblCart.clipsToBounds = true
    }
    ...
    func HideShowFilter()
    {
        print(CGRect(x:self.clFilterView.frame.origin.x, y:self.clFilterView.frame.origin.y, width:self.clFilterView.frame.size.width, height:self.clFilterView.contentSize.height))
...
//            }
        ...
        else if collectionView == self.clFilterView
        {
            ...
            {
                let indexOfA = self.arrSelected.firstIndex(of: strGiftID)
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize
    {
        if collectionView == self.clTopView
        {
            return CGSize(width: (collectionView.frame.size.width/CGFloat(self.arrTop.count)), height: 60)
        }
        else if collectionView == clFilterView
        {
            let arrValue = JSON(self.arrGiftCategory)
...
//            let strCity : String = self.arrBrands[indexPath.row].uppercased()
    ...
    @IBAction func btnResearch(_ sender: UIButton)
    {
        ...
        {
            Toast(text: "Please Enter the brand name").show()
        }
        else
        {
            if self.app.isConnectedToInternet()
    ...
    @IBAction func btnFilterActionDone(_ sender: UIButton)
    {
        ...
        {
            Toast(text: "Please Select atleast one filter category from above".localized).show()
        }
        else
        {
            if self.app.isConnectedToInternet()
    ...
    //MARK:- Home API
    func getHomeAPI(isLoading : Bool, isHideLoading : Bool)
    {
        var loadingNotification : MBProgressHUD!
...
//
    ...
    func getFilterVoucherAPI()
    {
        ...
        Alamofire.request("\(self.app.BaseURL)filter_voucher", method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: nil).responseJSON { response in
            debugPrint(response)
    ...
    func getAllDetails()
    {
        Alamofire.request("\(self.app.BaseURL)getAllActiveVoucher", method: .get, parameters: nil, encoding: JSONEncoding.default, headers: nil).responseData { (responce) in
    ...
    func SearchBrandAPI()
    {
        ...
        let parameters: Parameters = ["brand_name":self.strSearch]
        print(JSON(parameters))
        
        Alamofire.request("\(self.app.BaseURL)search_brand", method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: nil).responseJSON { response in
            debugPrint(response)
                ...
                if response.result.isSuccess == true
                {
                    if let value = response.result.value
                    {
                        ...
                        if strStatus == "1"
                        {
                            ...
                            let data : [Any] = self.json["data"].arrayValue
