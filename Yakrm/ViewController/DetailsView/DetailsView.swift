...
class DetailsView: UIViewController, UITableViewDelegate, UITableViewDataSource, MFMailComposeViewControllerDelegate, SFSafariViewControllerDelegate, MFMessageComposeViewControllerDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let Price = arrValue[indexPath.row]["voucher_price"].intValue
        let Discount = arrValue[indexPath.row]["discount"].intValue

        let DiscountPrice = Price * Discount / 100
        let PayValue = Price - DiscountPrice

        cell.lblValue.text = "\(Price)" + "SR".localized
        cell.lblDiscount.text = "\(Discount)%"
        cell.lblPay.text = "\(PayValue)" + "SR".localized
    ...
    @IBAction func btnShareAction(_ sender: UIButton) {
        if sender.tag == 1 {
            ...
            if self.isFavourite == 0 {
                ...
                let alertController = UIAlertController(title: "Are you sure", message: "You want to Remove from Favourites ?", preferredStyle: .alert)
                ...
                let OKAction = UIAlertAction(title: "Remove", style: .destructive) { (_: UIAlertAction!) in
...
//        loadingNotification.dimBackground = true
        ...
        AppWebservice.shared.request("\(self.app.BaseURL)getAllVoucherByBrandId", method: .post, parameters: parameters, headers: headers, loader: true) { (statusCode, response, error) in
            ...
            if statusCode == 200 {
                ...
                if strStatus == "1" {
                    ...
                    self.lblSub.text = self.json["description"].stringValue//"self.json [description] . stringValue dfgvdfbj fbghjiufhbiu ghybtry ghnbhrtuyhotuv uegtyeuyg gefrtwde3pq[keqw egf7ergf7"//
                    self.lblDetails.text = "Electronic and paper gifts"//self.json["description"].stringValue
                    ...
                    if self.arrVouchers.count > 0 {
                        for _ in 0...self.arrVouchers.count - 1 {
                            let check = Bool()
                    ...
                    let rectTitle = self.lblSub.text!.boundingRect(with: CGSize(width: self.lblSub.frame.size.width, height: CGFloat(50000)), options: .usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font: self.lblSub.font], context: nil)
...
//        loadingNotification.dimBackground = true
        ...
        AppWebservice.shared.request("\(self.app.BaseURL)add_to_cart", method: .post, parameters: parameters, headers: headers, loader: true) { (statusCode, response, error) in
            if statusCode == 200 {
                self.json = response!
                print(self.json)
